...
public class UploadController {
    ...
    // MultipartFile : 요청된 업로드 파일을 실제 업로드 하는 역할을 하는 객체
    // 사용법은 업로드 요청을 받으려면, 컨트롤러에 요청 패턴을 만들고 --> 처리 메서드를 정의하는데
    ...
    public ResponseEntity<List<UploadResultDTO>> uploadFile(MultipartFile[] uploadFiles) {
        ...
        for (MultipartFile uploadFile : uploadFiles) {
            ...
            //이를 근거로 업로드 파일의 타입을 조건 분리해서 image가 아닌경우엔
            //error 코드로 전송시킵니다.
            ...
            //위 SaveName을 MultipartFile가 수행하는 transferTo()에 맞게
            ...
            try {
                ...
                //썸네일 이미지생성..
                String theumbNailName = uploadPath + File.separator
                        + folderPath + File.separator + "s_" + uuid + "_" + fileName;

                File thumbFile = new File(theumbNailName);

                //추가 썸네일 이미지 생성 API 응용해서 이미지 생성하기
    ...
    /*
     * 메서드의 주요 내용은 아래와 같음.
     * 1. 사용자에게 ajax의 응답으로 이미지의 경로등을 보냅니다.
     * 2. 사용자는 해당 경로에 따른 이미지를 보게 되는데, 이때 실제 이미지가 필요합니다.
     * 3. 이미지는 사용자가 받아서 볼수 있는 형태로 보내져야 하는데, 다 아시다시피,
     * 자바에서 문자열을 제외한 모든 데이터는 byte 형태로 보내집니다.(사실 문자열도 내부적으로는
     * byte로 encoding, decoding 되어질뿐)
     *
     * 해서 해당 이미지를 byte[]로 보내는 작업도 병해압니다.
     */
    @GetMapping("/display")
    public ResponseEntity<byte[]> getFile(String fileName){
        ...
        try {
            ...
            //내부적으로 Servlet의 HttpHeader 객체를 확장한것임.
    ...
    // 아래 메서드는 요청자의 파일을 일자별로 구분지어서, 저장되는 경로를 년/월/일
    // 구분 후 해당 일에 저장하도록 정의 합니다.
    // 또한, 파일명을 UUID를 이용해서 변경하는 작업또한 작성합니다.
    private String makeFolder() {
        String str = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy/MM/dd"));

        // 저장되는 파일경로를 /-->플랫폼에 종속적인 경로 패스로 변경
        String folderPath = str.replace("/", File.separator);

        File uploadPathFolder = new File(uploadPath, folderPath);

        System.out.println("업로드된 파일 경로 : " + uploadPath);

        // mkDirs()를 호출해서 지정된 패스 경로에 맞게 동적으로 폴더를 생성합니다.
